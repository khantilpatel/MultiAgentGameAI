						  ...
						  float screenDepth, float screenNear)
{
	...
	swapChainDesc.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH; // Allow Auto Full screen Switching Alt-Enter
	...
	result = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, D3D11_CREATE_DEVICE_DEBUG,
	    ...
	    //result = D3D11CreateDevice( nullptr,                        // Use default graphics card
     ...
     //                           D3D11_CREATE_DEVICE_DEBUG,              // Device creation flags
     //                           &featureLevel,
					//			1,
     //                           D3D11_SDK_VERSION,           // SDK version
     //                           &m_device,                 // Device out
     //                           NULL,                      // Actual feature level created
     //                           &m_deviceContext );              // Context out
	...
	result = m_device->CreateBlendState(&blendStateDescription, &m_additiveEnableBlendingState);
	if(FAILED(result))
	{
		return false;
	}
...
void D3DClass::Shutdown()
{
	...
	if(m_depthEnableStencilState)
	{
		m_depthEnableStencilState->Release();
		m_depthEnableStencilState = 0;
	}
	...
	if(m_depthNoWriteStencilState)
	{
		m_depthNoWriteStencilState->Release();
		m_depthNoWriteStencilState = 0;
	}
...
void D3DClass::EnableAdditiveBlending()
{
	float blendFactor[4];
	...
	// Turn off the alpha blending.
	m_deviceContext->OMSetBlendState(m_additiveEnableBlendingState, blendFactor, 0xffffffff);
