...
void addToOpenList(uint offset, uint2 thisNode, uint7 parentGridNodeObject, uint parentGridId, uint2 targetNode, uint G_COST)
{
	...
	if (thisGridNode.status == _STATUS_UNREAD)
	{
		...
		insertPQ(offset, thisNode.x, thisNode.y, current_cost, H * 10, current_G_cost, parentGridId, _NODE_OPEN);
	...
	else if (thisGridNode.status == _NODE_OPEN)
	{
		...
		if (current_G_cost < thisGridNode.G_cost)
		{
			...
			// 4. calculate H cost
			int x_temp = abs(x1 - x2);
			int y_temp = abs(y1 - y2); //abs(asfloat(thisNode.y) - asfloat(targetNode.y));
			...
			gGridNodeListOut[offset + thisGridNodeID].parentId = parentGridId;
...
void main(uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupID)
{
	...
	uint sourceGridId = getGridIntegerCoordinate(nodeFirstLoc.x, nodeFirstLoc.y);
